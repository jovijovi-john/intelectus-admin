...
import type { PremiumCategoryPieSlice } from "../types";
import { DASHBOARD_HIGHLIGHT_CARD_CLASS } from "./dashboard-highlight-card";

type PremiumCategoriesPieChartProps = {
  data: PremiumCategoryPieSlice[];
};
  ...
  return (
    <div className="rounded-md border border-border bg-popover px-2.5 py-1.5 text-xs shadow-md">
      ...
      <p className="tabular-nums text-muted-foreground">
        {v.toLocaleString("pt-BR")} usuários ({formatPercent(v, total)})
      </p>
    </div>
  );
}

export function PremiumCategoriesPieChart({ data }: PremiumCategoriesPieChartProps) {
    ...
    return (
      <Card className={cn(DASHBOARD_HIGHLIGHT_CARD_CLASS)}>
        <CardHeader className="pb-2">
          <CardTitle className="text-base">Premium por categoria</CardTitle>
          <CardDescription>Usuários Pro por área de interesse.</CardDescription>
        </CardHeader>
  ...
  return (
    <Card className={cn(DASHBOARD_HIGHLIGHT_CARD_CLASS)}>
      <CardHeader className="pb-2">
        <CardTitle className="text-base">Premium por categoria</CardTitle>
        <CardDescription>
          Distribuição de usuários Pro por interesse, com paleta alinhada ao plano Premium.
        </CardDescription>
      </CardHeader>
      <CardContent className="flex flex-1 flex-col gap-3">
        <div className="relative mx-auto h-[220px] w-full max-w-[280px] shrink-0">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart margin={{ top: 8, right: 8, bottom: 8, left: 8 }}>
              ...
              <Tooltip
                content={(props) => <PremiumTooltip {...props} total={total} />}
              />
            </PieChart>
          </ResponsiveContainer>
        ...
        <ul className="mt-auto space-y-1.5 text-xs" aria-label="Detalhe por categoria">
          {data.map((d) => (
            <li
              key={d.id}
