...
import type { BloomPieSlice } from "../types";
import { DASHBOARD_HIGHLIGHT_CARD_CLASS } from "./dashboard-highlight-card";

type QuestionsBloomPieChartProps = {
  data: BloomPieSlice[];
};
...
export function QuestionsBloomPieChart({ data }: QuestionsBloomPieChartProps) {
    ...
    return (
      <Card className={cn(DASHBOARD_HIGHLIGHT_CARD_CLASS)}>
        <CardHeader className="pb-2">
          <CardTitle className="text-base">Questões por Bloom</CardTitle>
          <CardDescription>Distribuição do total pelo nível cognitivo.</CardDescription>
        </CardHeader>
  ...
  return (
    <Card className={cn(DASHBOARD_HIGHLIGHT_CARD_CLASS)}>
      <CardHeader className="pb-2">
        <CardTitle className="text-base">Questões por Bloom</CardTitle>
        <CardDescription>
          Total do acervo por nível cognitivo. Escala com o período selecionado.
        </CardDescription>
      </CardHeader>
      <CardContent className="flex flex-1 flex-col gap-3">
        <div className="relative mx-auto h-[220px] w-full max-w-[280px] shrink-0">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart margin={{ top: 8, right: 8, bottom: 8, left: 8 }}>
              ...
              <Tooltip
                content={(props) => <BloomTooltip {...props} total={total} />}
              />
            </PieChart>
          </ResponsiveContainer>
        ...
        <ul className="mt-auto space-y-1.5 text-xs" aria-label="Detalhe por nível Bloom">
          {data.map((d) => (
            <li
              key={d.tag}
